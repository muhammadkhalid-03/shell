 ...
 */
void handle_arg(char** arguments, int bg_flag) {  // handles single command

  if (strcmp(arguments[0], "\0") == 0) {  // check for empty input
    return;
  }
  if (strcmp(arguments[0], "exit") == 0) {  // check for exit command
    exit(EXIT_SUCCESS);
  }
  if (strcmp(arguments[0], "cd") == 0) {  // check for cd command
    chdir(arguments[1]);
    return;
  }
  pid_t child_id = fork();
  if (child_id == -1) {  // check for fork error
    perror("fork failed");
    exit(EXIT_FAILURE);
  }
...
int main(int argc, char** argv) {
  ...
  while (true) {
    ...
    while (arguments[cur_arg_pos] != NULL) {
      ...
      while (amp_position != NULL) {  // & exists
